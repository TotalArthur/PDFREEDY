...
- **Exact match** and **Fuzzy** are both on by default. They are tiers to include, not
  modes to choose between: everything is searched, and whole-tag hits are listed first.
  Untick Fuzzy and Exact becomes the one restrictive case — the tag has to *be* the
  whole string.
- **Fuzzy** is the last resort: it treats *every* character as uncertain, not just the
  ones OCR flagged. Those hits are banded `Possible`, badged `FUZZY`, sorted below
  everything else in their band, and ordered best-read first — a guess never looks like
  a certainty.
