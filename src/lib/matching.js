...
//
// Returns null, or { pos, len, confused, fuzzy, cost, subs, indels }. pos/len
// index the NORMALIZED candidate — a confusion match has no findable substring
// to search for afterwards, so the matcher has to say where it landed.
// cost/subs/indels describe how much damage the match had to absorb, which is
// what lets the UI say how sure it is rather than presenting every hit alike.
function matchWindow(norm, query) {
  ...
  if (query.exact) {
    if (norm === q) return { pos: 0, len: norm.length, confused: false, fuzzy: false, cost: 0, subs: 0, indels: 0 };
    ...
    if (hit && hit.pos === 0 && hit.len === norm.length) {
      return { pos: 0, len: norm.length, confused: hit.cost > 0, fuzzy: false,
  ...
  if (exactPos !== -1) {
    return { pos: exactPos, len: q.length, confused: false, fuzzy: false, cost: 0, subs: 0, indels: 0 };
  }

  const hit = confusableMatch(norm, needle, query);
  if (hit) {
    return { pos: hit.pos, len: hit.len, confused: true, fuzzy: false,
             cost: hit.cost, subs: hit.subs, indels: hit.indels };
  }
