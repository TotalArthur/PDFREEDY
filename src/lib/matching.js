...
//
// Returns null, or { pos, len, confused, fuzzy, cost, subs, unknowns, indels }.
// pos/len index the NORMALIZED candidate — a confusion match has no findable
// substring to search for afterwards, so the matcher has to say where it landed.
// The rest describe how much damage the match had to absorb, which is what lets
// the UI rank hits and say why rather than presenting them all alike.
function matchWindow(norm, query, opts) {
  ...
  if (query.exact) {
    if (norm === q) {
      return { pos: 0, len: norm.length, confused: false, fuzzy: false, cost: 0, subs: 0, unknowns: 0, indels: 0 };
    }
    ...
    if (hit && hit.pos === 0 && hit.len === norm.length) {
      return { ...describe(hit, false), pos: 0, len: norm.length };
    }
    return null;
  }
  ...
  if (exactPos !== -1) {
    return { pos: exactPos, len: q.length, confused: false, fuzzy: false, cost: 0, subs: 0, unknowns: 0, indels: 0 };
  }

  const hit = confusableMatch(norm, needle, { allowIndels: query.allowIndels, conf });
  if (hit) return describe(hit, false);
  ...
  if (query.fuzzy) {
    ...
    if (loose) return describe(loose, true);
  }
  return null;
}
